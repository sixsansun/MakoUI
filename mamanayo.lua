...
function MakoUI:CreateWindow(o)
    ...
    function api:AddTab(name)
        ...
        local function canvas()
            page.CanvasSize=UDim2.fromOffset(0,math.max(ll.AbsoluteContentSize.Y,rl.AbsoluteContentSize.Y)+4)
        end
        ...
        function tab:AddSection(title, side, headerIcon, questionMark)
            ...
            function s:AddSlider(text,min,max,default,callback,suffix)
                ...
                local val=New("TextLabel",{
                    Position=UDim2.fromOffset(105,0),Size=UDim2.fromOffset(55,31),
                    BackgroundTransparency=1,Text=tostring(value)..suffix,
                    TextColor3=C.TEXT,TextSize=12,Font=Enum.Font.GothamMedium,
                    TextXAlignment=Enum.TextXAlignment.Right,Parent=row
                })
                local track=New("Frame",{
                    Position=UDim2.new(0,158,.5,-2),Size=UDim2.new(1,-158,0,4),
                    BackgroundColor3=Color3.fromRGB(24,27,31),BorderSizePixel=0,
                    ClipsDescendants=false,Parent=row
                })
                Corner(track,4)
                Stroke(track,Color3.fromRGB(7,9,11),1,.35)

                local alpha=(value-min)/(max-min)
                local fillGlow=New("Frame",{
                    AnchorPoint=Vector2.new(0,.5),Position=UDim2.new(0,0,.5,0),
                    Size=UDim2.new(alpha,0,0,10),BackgroundColor3=accent,
                    BackgroundTransparency=.84,BorderSizePixel=0,ZIndex=1,Parent=track
                })
                Corner(fillGlow,6)
                local fill=New("Frame",{
                    Size=UDim2.new(alpha,0,1,0),BackgroundColor3=accent,
                    BorderSizePixel=0,ZIndex=2,Parent=track
                })
                Corner(fill,4)
                New("UIGradient",{
                    Color=ColorSequence.new({
                        ColorSequenceKeypoint.new(0,accent:Lerp(Color3.new(0,0,0),.68)),
                        ColorSequenceKeypoint.new(.62,accent),
                        ColorSequenceKeypoint.new(1,bright)
                    }),
                    Rotation=0,
                    Parent=fill
                ...
                local knobGlow=New("Frame",{
                    ...
                    BackgroundTransparency=.84,BorderSizePixel=0,ZIndex=3,Parent=track
                })
                Corner(knobGlow,16)
                ...
                local function setVisual(a)
                    ...
                    fillGlow.Size=UDim2.new(a,0,0,10)
